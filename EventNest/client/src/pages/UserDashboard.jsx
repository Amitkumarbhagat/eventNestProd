...
const UserDashboard = () => {
    ...
    if (loading) return <div className="text-center py-20 text-xl font-semibold text-gray-600">Loading dashboard...</div>;

    return (
        <div className="max-w-6xl mx-auto space-y-6">
            <div className="bg-white rounded-3xl shadow-sm p-6 sm:p-8 border border-gray-100 flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left gap-4 sm:gap-6">
                <div className="w-20 h-20 bg-gradient-to-br from-gray-900 to-gray-700 text-white rounded-2xl flex items-center justify-center text-3xl font-bold uppercase tracking-widest shrink-0 shadow-lg">
                    {user?.name.charAt(0)}
                </div>
            ...
            {bookings.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {bookings.map((booking) => (
                        <div key={booking._id} className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-100 flex flex-col hover:-translate-y-1">
                            ...
                            <div className="p-4 bg-gray-50/80 flex justify-between items-center shrink-0">
